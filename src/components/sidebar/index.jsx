...
function SidebarButton({name, propref}){
    return(
        <div className="w-full h-32 flex items-center justify-center">
            <p onClick={()=>{propref.current.scrollIntoView({ behavior: 'smooth', block: 'start' })}} className="cursor-pointer text-sm md:text-base rotate-[270deg] font-light text-[#121212]">{name}</p>
        </div>
    );
}
