function About({propref}) {
    return ( 
        <div  ref={propref} className="w-full h-auto md:pl-12 pl-6 flex pt-6 text flex-col">
            <p className="md:text-6xl text-3xl font-black text-stroke-1 text-[#121212]">About Me</p>
            {/* start cards list */}
            <div className="flex flex-col -ml-6 md:flex-row gap-8 items-center md:items-start py-6 justify-center">
                <AboutCard title="Education" >
                    <TimelineCardContent hasNext fromyear={2021} toyear="present" name="Polytechnic School of Sousse" degree="software engineering degree" />
...
function AboutCard({title, children}){
    return(
        <div className="w-70">
            ...
            <div className="bg-[#1e1e1e] w-72 mt-2 pb-4 px-4">
                {children}
            </div>
        </div>
    );
...
function TimelineCardContent({hasNext=false,fromyear,toyear,name,degree}){
    return(
        <div className="w-full pt-4">
            <div className="w-1/2 h-6 bg-lime-500 rounded-full flex items-center justify-center">
                <p className="text-[#1e1e1e] font-semibold text-xs">{fromyear} - {toyear}</p>
            </div>
            ...
            {hasNext && <div className="w-full border-b-[1px] border-dashed mt-6 border-stone-600" />}
